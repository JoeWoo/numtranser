        ...
        if $kanji["用词"][text[i]] != nil
          ...
          if temp == $N0
            if wait_words == "几" or wait_words == "好几"
              ...
              if wait_words.include?("点")
                ...
                if !wait_words.empty?
                  ...
                  if m.type == $N
                    result_text << highlight( wait_words,  (m.ex)+num2en(m.value), m.value.to_s)
            ...
            sub_words.each do
              ...
              if m.type == $N
                result_text << highlight(sub_word,(m.ex)+num2en(m.value), m.value.to_s)+"、"
              elsif m.type == $F && (m.denom == 100 or m.denom==1000)
                s = m.numer.to_s
                if m.numer.class == (1/1).to_r.class
                  s=m.numer.to_f.to_s
        ...
        if(i==0 && (char=="负" or char == "負" or char=="-"))
          ...
          if afterdecimal==1
            ...
            def highlight(str, english, value="-")
              @strid += 1
              "<a  data-toggle='tooltip' onmouseover=' titleshow(#{@strid})' onmouseout='titlehide(#{@strid})' title='#{english}' href='#' id='#{@strid}' class='highlight' m_value='#{value}'>#{str}</a>"
            end#end-func
